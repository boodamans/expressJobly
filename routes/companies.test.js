...
test("works", async function () {
  ...
  expect(company.handle).toBe("c1");
  expect(company.name).toBe("C1");
  expect(company.description).toBe("Desc1");
  expect(company.numEmployees).toBe(1);
  expect(company.logoUrl).toBe("http://c1.img");
  
  // Check if jobs is an array with length greater than or equal to 0
  expect(Array.isArray(company.jobs)).toBeTruthy();
  expect(company.jobs.length).toBeGreaterThanOrEqual(0);
});
