...
router.get("/:username", ensureLoggedIn, async function (req, res, next) {
  try {
    // Check if the request is from an admin or the user itself
    if (!res.locals.user.isAdmin && res.locals.user.username !== req.params.username) {
      throw new UnauthorizedError("Unauthorized");
    }

    const user = await User.get(req.params.username);
    const appliedJobs = await User.getAppliedJobs(req.params.username);
...
router.patch("/:username", ensureLoggedIn, async function (req, res, next) {
  try {
    // Logic to check if the request is from an admin or the user itself
    if (!res.locals.user.isAdmin && res.locals.user.username !== req.params.username) {
      throw new UnauthorizedError("Unauthorized");
    }
